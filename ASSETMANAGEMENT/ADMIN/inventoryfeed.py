...
def assigninventory():
    ...
    department_label = tk.Label(inventory_window, text="Department:")
    department_label.pack()
    department_entry = tk.Entry(inventory_window)
    department_entry.pack()

    mobile_label = tk.Label(inventory_window, text="Mobile:")
    mobile_label.pack()
    mobile_entry = tk.Entry(inventory_window)
    mobile_entry.pack()

    model_number_label = tk.Label(inventory_window, text="Model Number:")
    model_number_label.pack()
    model_number_entry = tk.Entry(inventory_window)
    model_number_entry.pack()
    ...
    date_of_allotment_label = tk.Label(inventory_window, text="Date of Allotment:")
    date_of_allotment_label.pack()
    date_of_allotment_entry = tk.Entry(inventory_window)
    date_of_allotment_entry.pack()

    empid_label = tk.Label(inventory_window, text="Emp ID:")
    empid_label.pack()
    empid_entry = tk.Entry(inventory_window)
    empid_entry.pack()

    assign_button = tk.Button(inventory_window, text="Assign", command=assign_inventory_to_user)
    assign_button.pack()
...
def inventorywindow():
    ...
    inventory_window = Toplevel()
    Insertinventorybutton = Button(inventory_window, text="INSERT NEW INVENTORY", command=lambda: insertinventory())
    Insertinventorybutton.pack()
    ViewInventory = Button(inventory_window, text="VIEW STOCK", command=viewinventory.viewinventries)
    ViewInventory.pack()
    UpdateInventoryButton = Button(inventory_window, text="UPDATE EXISTING INVENTORY",command=editinventory.editinventries)
    UpdateInventoryButton.pack()
    Assigninventory=Button(inventory_window,text="ASSIGN INVENTORY",command=lambda:assigninventory())
    Assigninventory.pack()
    Assignviewinventory=Button(inventory_window,text="VIEW ASSIGN INVENTORY",command=lambda:assignviewinventory())
